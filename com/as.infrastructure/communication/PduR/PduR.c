...
void PduR_Init (const PduR_PBConfigType* ConfigPtr) {
	...
	if (PduRState != PDUR_UNINIT) {
		...
		ASLOG(LOW, ("--Initialization of PDU router--\n"));
		...
		/*if (failed) {
			...
			ASLOG(LOW, ("--Initialization of PDU router failed--\n"));
		}*/

		// The initialization succeeded!
			PduRState = PDUR_ONLINE;
			ASLOG(LOW, ("--Initialization of PDU router completed --\n"));
	}
...
void PduR_BufferQueue(PduRTxBuffer_type *Buffer, const uint8 * SduPtr) {
	...
	if (PduR_BufferIsFull(Buffer)) { // Buffer is full
		...
		Buffer->NrItems++;
		ASLOG(LOW, ("\tBuffer %d: Queued data %d. Status: NrItems %d,  First %d,  Last %d\n",  Buffer->BufferId,  *SduPtr,  Buffer->NrItems,  *Buffer->First, *Buffer->Last));
...
void PduR_BufferDeQueue(PduRTxBuffer_type *Buffer, uint8 *SduPtr) {
	...
	if (Buffer->NrItems > 0) {
		...
		Buffer->NrItems--;
		ASLOG(LOW, ("\tBuffer %d: DeQueueing data. Status: NrItems %d,  First %d,  Last %d\n",  Buffer->BufferId,  Buffer->NrItems,  *Buffer->First, *Buffer->Last));
	} else {
		ASLOG(LOW, ("\tBuffer %d: Buffer is empty,  nothing to dequeue!\n", Buffer->BufferId));
	}
    Irq_Restore(state);
}

void PduR_BufferFlush(PduRTxBuffer_type *Buffer) {
	ASLOG(LOW, ("\tBuffer %d: Flushing!\n", Buffer->BufferId));
    imask_t state;
    Irq_Save(state);
	Buffer->NrItems = 0;
